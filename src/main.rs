...
fn bissecao(mut a: f32, mut b: f32, mut parada: f32, tolerancia: f32) {
...
fn false_position(a: f32, b: f32, mut parada: f32, tolerancia: f32) {
    ...
    loop {
        ...
        if parada >= 10.0 || f32::abs(fxk) < tolerancia {
            table1.printstd();
            break;
        }
    }
...
fn main() {
    loop {
        ...
        let parada: f32 = match parada.trim().parse() {
            Ok(num) => num,
            Err(_) => {
                println!("Por favor, digite um número!");
                continue;
        ...
        bissecao(a, b, parada, tolerancia);
        false_position(a, b, parada, tolerancia);
